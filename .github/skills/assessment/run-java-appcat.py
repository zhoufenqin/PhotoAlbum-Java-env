#!/usr/bin/env python3
"""run_java_appcat.py — Cross-platform Java AppCAT assessment runner.

This script is shipped inside the assessment skill and runs in the coding agent's
workspace. It runs ``appcat analyze`` parameterized from the assessment config and
injects assessment metadata into the resulting report.json — using only the Python
...
Usage:
    python run_java_appcat.py [--workspace-path PATH] [--config FILE] [--reports-dir DIR]
...
def run_appcat(executable, args):
    ...
    try:
        ...
        log(f"appcat process error ({exc}).")
...
def main():
    ...
    if not executable:
        fail(
            f"No AppCAT executable found under {appcat_home}. "
            "The skill must download and extract AppCAT there first."
        )
    if os.name != "nt":
        try:
...
if __name__ == "__main__":
    try:
        ...
        import traceback
        fail(f"Unexpected error: {traceback.format_exc() if exc else exc}")
