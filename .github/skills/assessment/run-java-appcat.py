...
def _extract_stripped(archive, dest_dir):
    # Extract archive into dest_dir, stripping the single top-level folder.
    #
    # Prefer native tar: bsdtar (Windows / macOS runners) and GNU tar (Linux
    # runners) both ship by default. bsdtar auto-detects format and extracts .zip
    # too, while our POSIX archives are always .tar.gz which GNU tar handles. tar
    # unpacks the whole bundle in ONE native process with --strip-components=1
    # removing the top folder. This is orders of magnitude faster than writing the
    # thousands of files one-by-one from Python: the appcat bundle embeds a full
    # JDK, and pure-Python extraction was observed taking 20+ minutes on the
    # GitHub-hosted Windows runner (Defender scans each file as Python writes it),
    # which is what made the assessment appear to hang. The archive is already
    # sha256-verified against our manifest, so its contents are trusted.
    ...
    try:
        ...
        detail = exc.stderr.decode("utf-8", "replace").strip() if isinstance(exc, subprocess.CalledProcessError) and exc.stderr else exc
        log(f"Native tar extraction unavailable/failed ({detail}); using Python fallback.")
