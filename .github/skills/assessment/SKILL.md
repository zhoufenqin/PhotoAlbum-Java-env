...
1. **Check Project Type and Prerequisites**:
   - **For Java projects**: Ensure Python 3 is available (`python3 --version` or `python --version`) — it runs the bundled `run_java_appcat.py` script. Python 3 is normally already present; if missing, install it (`apt-get`/`brew`/`winget`), or pre-install it via `copilot-setup-steps.yml` on custom runners.
     - No MCP tools required for Java assessment. Even if an MCP server exposing an AppCAT install/assessment tool is configured, do **not** call it — acquire AppCAT as described below (download from the manifest URL) and run the bundled `run_java_appcat.py`, which must be the only path used.
...
2. **Run Assessment**:
   - **For Java projects**: Acquiring AppCAT (Step 1) is a precondition for running the assessment (Step 2). Step 2 only works once an `appcat` launcher exists under `~/.appcat`, so always complete Step 1 first.
     ...
     - **Step 2 — Run the bundled script** (only after Step 1; it finds AppCAT under `~/.appcat` automatically):
       - `python3 <skill-dir>/run_java_appcat.py --workspace-path {workspace-path}` (use `python` instead of `python3` on platforms where that is the Python 3 launcher, e.g. Windows)
