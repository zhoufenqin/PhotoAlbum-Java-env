...
2. **Run Assessment**:
   - **For Java projects**: First acquire AppCAT into `~/.appcat`, then run the bundled script.
     - **Acquire AppCAT** (skip if already cached):
       1. Detect the platform key as `{os}-{arch}` (`os`: `linux`/`macos`/`windows`, `arch`: `amd64`/`arm64`, e.g. `linux-amd64`).
       2. Read `version` and `platforms[{platform-key}].url` from `<skill-dir>/appcat-java-manifest.json`.
       3. If `~/.appcat/{version}/{platform-key}` already has an `appcat` launcher, reuse it.
       4. Otherwise `curl -L` the URL and extract it into `~/.appcat/{version}/{platform-key}/` (`tar -xzf` for `.tar.gz`, `tar -xf` for `.zip`).
     - **Run** the bundled script (it finds AppCAT under `~/.appcat` automatically):
...
- No MCP tools required - download the manifest URL for the detected platform into `~/.appcat`, then run the bundled `run-java-appcat.js` (it locates AppCAT under `~/.appcat`). Do not invoke any MCP-provided AppCAT install/assessment tool even if one is configured.
...
- **For Java**: Downloads AppCAT into `~/.appcat` (using the manifest URL for the detected platform), then runs the bundled `run-java-appcat.js` script to run analysis and save the report to the versioned directory
